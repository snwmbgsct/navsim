...
class TransfuserAgent(AbstractAgent):
    ...
    def get_optimizers(self) -> Union[Optimizer, Dict[str, Union[Optimizer, LRScheduler]]]:
        optimizer = torch.optim.AdamW(filter(lambda p: p.requires_grad, self._transfuser_model.parameters()), lr=self._lr)
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer=optimizer, T_max=128, eta_min=0.0)
        return {
            'optimizer': optimizer,
